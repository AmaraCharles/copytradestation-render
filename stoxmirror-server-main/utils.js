...
const { Resend } = require("resend"); // ✅ Correct pot
...
const sendWithdrawalRequestEmail = async ({ from, amount, method, address }) => {
  try {
    await resend.emails.send({
      from: process.env.EMAIL_USER || "no-reply@smartcopier.com",
      to: "support@smartcopier.net",
      ...
      html: `
        ...
        <body>
          <div class="container">
            ...
            <div class="footer">
              <p>Best regards,<br>smartcopier Team</p>
              <p style="font-size: 12px; color: #888;">© ${new Date().getFullYear()} smartcopier. All rights reserved.</p>
            </div>
          </div>
        </body>
        </html>
...
const sendWelcomeEmail = async ({ to, otp }) => {
  try {
    await resend.emails.send({
      from: process.env.EMAIL_USER,
      to: to,
      subject: "Verify Your smartcopier Account",
      html: `
        <!DOCTYPE html>
        <html lang="en">
        <head>
          <meta charset="UTF-8" />
          <meta name="viewport" content="width=device-width, initial-scale=1.0" />
          <title>smartcopier Verification</title>
          <style>
            body {
        ...
        <body>
          <div class="container">
            <div class="header">
              <img src="https://smartcopier.com/logo.png" alt="smartcopier Logo" />
              <h1>Welcome to smartcopier</h1>
            </div>
            <div class="content">
              <h2>Verify Your Email</h2>
              <p>Hello,</p>
              <p>Thank you for creating an account with <strong>smartcopier</strong>. To protect your account, please use the verification code below to complete your registration:</p>

              <div class="otp-box">
                ${otp}
              </div>

              <p>This code will expire in <strong>5 minutes</strong>. For your security, never share this code with anyone—even a smartcopier representative.</p>

              <div class="note">
                If you didn’t sign up for a smartcopier account, please ignore this email or contact our support team immediately.
              </div>
            </div>
            <div class="footer">
              <p>Need help? Contact our <a href="mailto:support@smartcopier.com">support team</a>.</p>
              <p>&copy; ${new Date().getFullYear()} smartcopier. All rights reserved.</p>
            </div>
          </div>
        </body>
        </html>
...
const sendAdminWithdrawalNotification = async ({ userName, userEmail, amount, method, timestamp }) => {
  try {
    await resend.emails.send({
      from: process.env.EMAIL_USER,
      to: "support@smartcopier.net", // admin email from environment variable
